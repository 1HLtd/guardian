...
sub get_exim_queue {
	...
	if ( -f '/etc/init.d/exim4' ) {
		open C, '<', '/etc/init.d/exim4' or die "Cannot open /etc/init.d/exim4: $!\n";
		while (<C>) {
			next if ($_ !~ /^\s*QUEUEINTERVAL=.*$/);
			$config_ref->{'exim_queue_time'} = $1 if ($_ =~ /^^\s*QUEUEINTERVAL='([0-9]+[h|m|s])'$/);
			last;
		}
		close C;
	}
...
sub get_exim_info {
	...
	$config_ref->{'has_spamd'} = 0 if ( -e '/etc/spamddisable' || ! -x '/usr/bin/spamd' );
